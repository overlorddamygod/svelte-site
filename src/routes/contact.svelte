...
<Layout _title="Contact">
  <h2 class="text-center text-3xl my-5">Contact Us</h2>
  {#if response.message != ""}
    <div
      class="text-white rounded-md text-center py-2 my-5"
      class:bg-green-400={response.success}
      class:bg-red-400={!response.success}
    >
      {response.message}
    </div>
  {/if}
  ...
  <button
    class="py-2 px-4 bg-purple-500 text-white font-semibold rounded-lg shadow-md focus:outline-none"
    type="submit"
